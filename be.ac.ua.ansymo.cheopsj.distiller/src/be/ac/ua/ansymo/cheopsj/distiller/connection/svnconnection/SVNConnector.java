...
public class SVNConnector extends Connector {
	...
	// The clientmanager, used to do everything related to svn.
	private SVNClientManager clientManager;
	
	// The repository on which we are working.
	 ...
	 *
	 **/
	public SVNConnector(String userName, String userPassword, String url) {
	...
	public void getCommitMessage(File file, long revisionNumber, LogEntryHandler handler){
		assert (handler instanceof SVNLogEntryHandler);
		
		try {
			SVNLogClient logClient = clientManager.getLogClient();
			SVNRevision revision = SVNRevision.create(revisionNumber);
			File[] files = {file};
			logClient.doLog(files, revision, revision, true, true, 1, 
					(SVNLogEntryHandler) handler);
		} catch (SVNException e) {
			//e.printStackTrace();
		}
	}
